#!/usr/bin/env python3
"""Launch B-aligned B + SigLIP training: 16-NPU smoke, then one 64-NPU job per arm."""
...
from utils.b_siglip_protocol import config_path, validate_b_siglip_config
...
def launch_plan(variant: str, *, smoke: bool, label: str, steps: int, environment: dict,
    ...
    contract = validate_b_siglip_config(config)
    ...
    if smoke:
        ...
        if not 0 <= rank < machines or int(environment.get("PET_NNODES", "0")) != machines:
            raise ValueError("formal B + SigLIP training requires PET_NODE_RANK=0..3 and PET_NNODES=4")
        if int(environment.get("PET_NPROC_PER_NODE", "0")) not in (0, 16):
            raise ValueError("formal B + SigLIP training requires 16 NPUs per instance")
    ...
    if not smoke:
        ...
        if not address or not port:
            raise ValueError("formal B + SigLIP training requires the platform master address and port")
    ...
    preflight = [
        sys.executable, "scripts/validate_unified_b_siglip.py",
        "--config", config_file, "--require-npu-count", "16",
    ]
    if rank == 0:
        preflight.append("--assets")
    return {"contract": contract, "smoke": smoke, "world_size": world,
            "resume_step": resume_step, "resume_from_checkpoint": resume_from_checkpoint,
...
def main():
    ...
    parser.add_argument("--variant", choices=("b-siglip",), default="b-siglip")
    parser.add_argument("--smoke", action="store_true")
    parser.add_argument("--label", default="r1")
    parser.add_argument("--steps", type=int, default=6)
    parser.add_argument("--resume-from-checkpoint", help="Complete checkpoint from this same run and world size")
    ...
    if (audit_root / "training.log").exists():
        raise FileExistsError(f"refusing to overwrite existing launch log: {audit_root}")
