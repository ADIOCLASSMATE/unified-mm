#!/usr/bin/env python3
"""Validate the S2 training contract and B's shared assets without content hashing."""
import argparse
import glob
import json
import os
...
from utils.showo2_unified_protocol import validate_s2_config, parameter_report
...
def main():
    ...
    report = {"schema": "showo2_unified_preflight_v1", "contract": validate_s2_config(config),
    ...
    if args.assets:
        ...
        if config.model.s2_use_siglip:
            ...
            p = _require_file(str(Path(config.model.s2_siglip_path) / "model.safetensors"), "SigLIP")
