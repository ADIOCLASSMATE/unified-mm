"""Closed position contracts for the contextual Selfless Flow head."""
...
FLOW_HEAD_POSITION_SPECS = {
    ...
    "FH1": FlowHeadPositionSpec(
        "FH1", "additive_2d", "additive_2d", "row_col_2d"
    ),
    "FH2": FlowHeadPositionSpec("FH2", "additive_2d", "none", "none"),
    "FH3": FlowHeadPositionSpec("FH3", "additive_2d", "none", "row_col_2d"),
...
def resolve_model_flow_head_position(
    ...
    """Resolve the five preregistered modes and persist their exact contract."""
    ...
    if has_variant:
        ...
        if spec is None:
            raise ValueError(
                "Flow-head position flags do not match a preregistered FH0--FH4 "
                f"system: {modes}."
            )
    else:
...
def resolve_flow_head_position_config(
    ...
    experiment = _get(config, "experiment", {})
    ablation_id = str(_get(experiment, "ablation_id", "")).strip().upper()
    if ablation_id.startswith("FH"):
        expected = normalize_flow_head_position_variant(ablation_id)
        if spec.variant != expected:
            raise ValueError(
                f"experiment.ablation_id={ablation_id} conflicts with "
                f"image_flow_position_variant={spec.variant}."
            )
        backbone = str(_get(model, "image_backbone_variant", "E2-Q1"))
        if backbone != "E2-Q1":
            raise ValueError(
                "The primary FH0--FH4 matrix requires "
                f"image_backbone_variant=E2-Q1, got {backbone!r}."
            )
        _set(experiment, "flow_head_position_variant", spec.variant)
    return spec, axis_dims
