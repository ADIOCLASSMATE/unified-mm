...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.image_flow_head = FlowLoss(
            ...
            flow_head_variant=getattr(config, "image_flow_head_variant", "DF0"),
        )
    ...
    def sample_image_latents_single_stream(
        ...
        if dynamic_flow_head and k != 1:
            raise ValueError(
                "DF1/DF2 incremental content-cache generation requires "
                "parallel_rate=1."
            )
        if dynamic_flow_head and order_strategy in latent_level_strategies:
            raise ValueError(
                "DF1/DF2 formal generation does not support speculative "
                "latent-level candidate scoring."
            )
        if dynamic_flow_head and bool(filled.any().item()):
            raise ValueError(
                "DF1/DF2 incremental generation currently requires an empty "
                ...
                if dynamic_flow_head:
                    if count != 1:
                        raise AssertionError(
                            "DF1/DF2 cache commit must append one token at a time."
                        )
        ...
        if return_trace:
            trace = {
                ...
                "flow_head_variant": getattr(
                    self.image_flow_head, "flow_head_variant", "DF0"
                ),
