...
from scripts.flow_head_position_ablation import (  # noqa: E402
    compact_provenance as compact_flow_head_position_provenance,
    file_sha256 as flow_head_position_file_sha256,
    is_flow_head_position_config,
    load_and_validate_training_provenance as load_flow_head_position_training_provenance,
    output_run_dir as flow_head_position_output_run_dir,
    provenance_path as flow_head_position_provenance_path,
    validate_ablation_config as validate_flow_head_position_config,
)
from scripts.dual_stream_flow_head_ablation import (  # noqa: E402
    is_dual_stream_flow_head_config,
    training_artifacts as dual_stream_training_artifacts,
    validate_ablation_config as validate_dual_stream_flow_head_config,
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fail unless the checkpoint, FH0--FH4 position contract, training "
            "provenance, paired initialization, final validation, and formal "
            "10K evaluator protocol all validate."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fail unless the checkpoint, DF1/DF2 architecture/cache contract, "
            "paired initialization, final validation, and formal evaluation "
...
def flow_head_position_training_artifacts(
    config,
    *,
    model_path: str,
) -> dict[str, object]:
    """Validate the complete train-to-eval evidence for one formal FH cell."""

    validate_flow_head_position_config(config)
    final_global_step = int(config.training.max_train_steps)
    run_dir = flow_head_position_output_run_dir(config)
    checkpoint_dir = run_dir / f"checkpoint-{final_global_step}"
    metadata_path = checkpoint_dir / "metadata.json"
    ema_state_path = checkpoint_dir / "ema_state.pt"
    validation_path = (
        run_dir / f"validation_metrics_step_{final_global_step}.json"
    )
    hf_weights_path = Path(model_path) / "model.safetensors"
    provenance_path = flow_head_position_provenance_path(config)
    try:
        metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError(
            f"Missing or invalid final FH checkpoint metadata: {metadata_path}"
        ) from exc
    if metadata.get("global_step") != final_global_step:
        raise ValueError(
            f"FH checkpoint global_step={metadata.get('global_step')!r}; "
            f"expected {final_global_step}"
        )
    missing = [
        str(path)
    ...
    if missing:
        raise ValueError(f"Missing final FH training artifacts: {missing}")
    provenance = load_flow_head_position_training_provenance(
        provenance_path,
        config=config,
    )
    ...
    if metadata.get("flow_head_position") != expected_binding:
        raise ValueError(
            "Final FH checkpoint provenance binding mismatch: "
            f"expected={expected_binding!r}, "
            f"actual={metadata.get('flow_head_position')!r}"
        )
    stored_model_config = metadata.get("model_config")
    if not isinstance(stored_model_config, Mapping):
        raise ValueError("Final FH checkpoint is missing its model config")
    for key in (
        ...
        expected_value = config.model.get(key)
        expected = (
            OmegaConf.to_container(expected_value, resolve=True)
            if OmegaConf.is_config(expected_value)
            else expected_value
        )
        if stored_model_config.get(key) != expected:
            raise ValueError(
                f"Final FH checkpoint model.{key} drifted: "
                f"expected={expected!r}, actual={stored_model_config.get(key)!r}"
            )
    hf_provenance_path = Path(model_path) / provenance_path.name
    hf_provenance = load_flow_head_position_training_provenance(
        hf_provenance_path,
        config=config,
        expected_sha256=provenance_sha256,
    )
    if hf_provenance != provenance:
        raise ValueError("EMA HF FH provenance copy differs from training evidence")
    validation = json.loads(validation_path.read_text(encoding="utf-8"))
    if (
        validation.get("schema") != "selfless_flow_validation_metrics_v1"
        or int(validation.get("global_step", -1)) != final_global_step
        or validation.get("ablation_id") != str(config.experiment.ablation_id)
        or int(validation.get("training_seed", -1)) != int(config.training.seed)
    ):
        raise ValueError("Final FH validation metrics identity mismatch")
    validation_values = validation.get("metrics")
    if not isinstance(validation_values, Mapping):
        raise ValueError("Final FH validation metrics payload is missing")
    required_validation = (
        "val/flow/v_mse",
        "val/flow/context_0_v_mse",
        "val/flow/context_1_v_mse",
        "val/flow/context_2_4_v_mse",
        "val/flow/context_5_16_v_mse",
        "val/flow/context_17_64_v_mse",
        "val/flow/context_65_plus_v_mse",
        "val/flow/context_0_gate_abs",
        "val/flow/context_65_plus_gate_abs",
        "val/flow/context_0_attention_entropy",
        "val/flow/context_1_attention_entropy",
        "val/flow/context_65_plus_attention_entropy",
        "val/flow/context_0_attention_distance",
        "val/flow/context_1_attention_distance",
        "val/flow/context_65_plus_attention_distance",
    )
    for key in required_validation:
        require_finite_metric_scalar(
            validation_values.get(key),
            label=f"FH final validation {key}",
        )
    return {
        "schema": "selfless_flow_head_position_training_artifacts_v1",
        "final_global_step": final_global_step,
        "provenance": compact_flow_head_position_provenance(provenance),
        "validation_metrics_path": str(validation_path),
        "artifacts": {
            "checkpoint_metadata_path": str(metadata_path),
            "checkpoint_metadata_sha256": flow_head_position_file_sha256(
                metadata_path
            ),
            "ema_state_path": str(ema_state_path),
            "ema_state_size_bytes": int(ema_state_path.stat().st_size),
            ...
            "hf_model_weights_path": str(hf_weights_path),
            "hf_model_weights_size_bytes": int(hf_weights_path.stat().st_size),
            "hf_model_weights_sha256": _distributed_file_sha256(
                hf_weights_path
            ),
            "provenance_path": str(provenance_path),
            "hf_provenance_path": str(hf_provenance_path),
        },
    }
...
def main():
    ...
    if (
        args.require_image_embedder_ablation_protocol
        or args.require_flow_head_position_ablation_protocol
        or args.require_dual_stream_flow_head_ablation_protocol
    ):
    ...
    if (
        ...
        or args.require_flow_head_position_ablation_protocol
        or args.require_dual_stream_flow_head_ablation_protocol
    ) and args.debug_finite_generation:
        raise ValueError("debug generation flags are forbidden by the formal ablation protocol")
    if args.require_flow_head_position_ablation_protocol:
        if not is_flow_head_position_config(config):
            raise ValueError(
                "--require_flow_head_position_ablation_protocol requires an FH config"
            )
        validate_flow_head_position_config(config)
    if args.require_dual_stream_flow_head_ablation_protocol:
        if not is_dual_stream_flow_head_config(config):
            raise ValueError(
                "--require_dual_stream_flow_head_ablation_protocol requires "
                "a DF1/DF2 config"
            )
        validate_dual_stream_flow_head_config(config)
    ...
    canonical_pairing_enabled = (
        ...
        or is_flow_head_position_config(config)
        or is_dual_stream_flow_head_config(config)
    )
    ...
    elif args.require_flow_head_position_ablation_protocol:
        training_protocol = {
            "flow_head_position": flow_head_position_training_artifacts(
                config,
                model_path=(
                    args.model_path_override or str(config.model.model_path)
                ),
            )
        }
    elif args.require_dual_stream_flow_head_ablation_protocol:
        training_protocol = {
            "dual_stream_flow_head": dual_stream_training_artifacts(
                config,
                model_path=(
                    args.model_path_override or str(config.model.model_path)
                ),
    ...
    if (
        (
            args.require_flow_head_position_ablation_protocol
            or args.require_dual_stream_flow_head_ablation_protocol
        )
        and hasattr(model.image_flow_head, "guidance_diagnostics")
    ):
        local_guidance = model.image_flow_head.guidance_diagnostics()
        for tag in ("0", "1", "2_4", "5_16", "17_64", "65_plus"):
            entry = local_guidance.get(tag)
            local_sum = (
                float(entry["sum"].detach().float().item())
                if isinstance(entry, Mapping)
                else 0.0
            )
            local_count = (
                float(entry["count"].detach().float().item())
                if isinstance(entry, Mapping)
                else 0.0
            )
            global_sum = reduce_sum(local_sum, device)
            global_count = reduce_sum(local_count, device)
            if global_count <= 0:
                raise ValueError(
                    f"formal FH guidance diagnostics are missing context bucket {tag}"
                )
            guidance_diagnostics[tag] = require_finite_metric_scalar(
                global_sum / global_count,
                label=f"guidance velocity delta RMS context bucket {tag}",
            )
    ...
    results = {
        ...
        "architecture": {
            ...
            "flow_head": {
                ...
                "variant": str(
                    config.model.get("image_flow_head_variant", "DF0")
                ),
