...
def save_checkpoint(model, config, accelerator, global_step):
    ...
    if accelerator.is_main_process:
        ...
        if (
            str(config.experiment.get("ablation_study", "")).strip().lower()
            == "flow_head_position"
        ):
            metadata["flow_head_position"] = {
                ...
                "provenance_path": str(
                    config.experiment.flow_head_position_provenance_path
                ),
            ...
            metadata["dual_stream_flow_head"] = {
                "ablation_id": str(config.experiment.ablation_id),
                "phase": str(config.experiment.ablation_phase),
                "config_fingerprint": str(
                    config.experiment.config_fingerprint
                ...
                "provenance_path": str(
                    config.experiment.dual_stream_flow_head_provenance_path
                ),
                "provenance_sha256": str(
                    config.experiment.dual_stream_flow_head_provenance_sha256
                ),
            }
        elif str(config.experiment.get("ablation_phase", "screen")) == "confirmation":
