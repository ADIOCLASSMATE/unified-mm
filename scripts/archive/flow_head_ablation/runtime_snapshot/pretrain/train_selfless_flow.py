...
from scripts.flow_head_position_ablation import (
    build_training_provenance as build_flow_head_position_training_provenance,
    ...
    write_training_provenance as write_flow_head_position_training_provenance,
)
from scripts.dual_stream_flow_head_ablation import (
    build_training_provenance as build_dual_stream_training_provenance,
    is_dual_stream_flow_head_config,
    load_and_validate_training_provenance as load_dual_stream_training_provenance,
    provenance_path as dual_stream_provenance_path,
    write_training_provenance as write_dual_stream_training_provenance,
)
...
def _uses_paired_image_initialization(config) -> bool:
    return (
        is_flow_head_position_config(config)
        or is_dual_stream_flow_head_config(config)
        or is_confirmation_config(config)
...
def _save_ema_hf_model(ema_model, tokenizer, config, accelerator, global_step) -> None:
    ...
    if is_flow_head_position_config(config):
        provenance_path = Path(
            str(config.experiment.flow_head_position_provenance_path)
        )
        shutil.copy2(provenance_path, save_path / provenance_path.name)
    elif is_dual_stream_flow_head_config(config):
        provenance_path = Path(
            str(config.experiment.dual_stream_flow_head_provenance_path)
        )
        shutil.copy2(provenance_path, save_path / provenance_path.name)
    elif is_confirmation_config(config):
...
def main():
    ...
    if is_flow_head_position_config(config):
        ...
        if accelerator.is_main_process:
            if provenance_path.exists():
                raise FileExistsError(
                    "Refusing to overwrite FH training provenance: "
                    f"{provenance_path}"
                )
            provenance = build_flow_head_position_training_provenance(
                config=config,
                model=model,
                train_loader=train_dataloader,
                special_token_ids=_special_token_ids(config),
            )
            write_flow_head_position_training_provenance(
                provenance_path,
                provenance,
            )
        accelerator.wait_for_everyone()
        stored = json.loads(provenance_path.read_text(encoding="utf-8"))
        provenance_digest = str(stored.get("provenance_sha256", ""))
        load_flow_head_position_training_provenance(
            provenance_path,
            config=config,
            expected_sha256=provenance_digest,
        )
        config.experiment.flow_head_position_provenance_path = str(
            provenance_path
        )
        config.experiment.flow_head_position_provenance_sha256 = (
            provenance_digest
        )
        logger.info(
            "Bound FH training provenance "
            f"sha256={provenance_digest}"
        )
    elif is_dual_stream_flow_head_config(config):
        provenance_path = dual_stream_provenance_path(config)
        if accelerator.is_main_process:
            if provenance_path.exists():
                raise FileExistsError(
                    "Refusing to overwrite DF training provenance: "
                    f"{provenance_path}"
                )
            provenance = build_dual_stream_training_provenance(
                config=config,
                model=model,
                train_loader=train_dataloader,
                special_token_ids=_special_token_ids(config),
            )
            write_dual_stream_training_provenance(
                provenance_path,
                provenance,
            )
        accelerator.wait_for_everyone()
        stored = json.loads(provenance_path.read_text(encoding="utf-8"))
        provenance_digest = str(stored.get("provenance_sha256", ""))
        load_dual_stream_training_provenance(
            provenance_path,
            config=config,
            expected_sha256=provenance_digest,
        )
        config.experiment.dual_stream_flow_head_provenance_path = str(
            provenance_path
        )
        config.experiment.dual_stream_flow_head_provenance_sha256 = (
            provenance_digest
        )
        logger.info(
            "Bound DF training provenance "
            f"sha256={provenance_digest}"
        )
    elif is_confirmation_config(config):
        provenance_path = (
