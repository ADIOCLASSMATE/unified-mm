#!/usr/bin/env python3
"""Reload complete S2 raw/EMA exports and check NPU gradients and all generation tasks."""
import argparse
import gc
import json
...
from utils.showo2_unified_protocol import config_path
from utils.utils import load_model_tokenizer


def check_gradients(model, batch):
    model.train()
    result = model(X0_input_ids=batch["input_ids"], token_types=batch["token_types"],
...
def main():
    ...
    for kind in ("current", "ema"):
        ...
        assert trace["backbone_calls"] == 40 and trace["backbone_kv_cache_enabled"] is False
    ...
    result = {"schema": "showo2_generation_smoke_v1", "passed": True, "variant": args.variant, "reports": reports}
