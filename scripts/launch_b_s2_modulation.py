#!/usr/bin/env python3
"""Launch B + S2 modulation on 64 NPUs, or its bounded 16-NPU smoke."""
...
from utils.b_s2_modulation_protocol import CONFIG, RUN, validate_b_s2_modulation_config


def launch_plan(*, smoke, label, steps, environment, resume=None):
    config = OmegaConf.load(ROOT / CONFIG)
    contract = validate_b_s2_modulation_config(config)
    ...
    preflight = [sys.executable, "scripts/validate_b_s2_modulation.py", "--require-npu-count", "16"]
    if rank == 0:
