...
from models.modeling_model.modeling_selfless_flow_latentmix import Qwen3ForCausalLM as LatentMixQwen3ForCausalLM
...
def load_model_tokenizer(config: OmegaConf, logger=None):
    ...
    if logger is not None:
        logger.info("Using latent-mix flow model implementation.")
    ...
    if config.training.from_scratch:
        if logger is not None:
            logger.info(f"Initializing latent-mix model from scratch using config from: {config.model.model_path}")
        model = LatentMixQwen3ForCausalLM(model_config).to(dtype=torch.bfloat16)
    else:
        if logger is not None:
            logger.info(f"Loading pretrained weights into latent-mix model from: {config.model.model_path}")
        model = LatentMixQwen3ForCausalLM.from_pretrained(
