...
from .mar_diffusion import create_diffusion
...
class DiffLoss(nn.Module):
    ...
    def forward(self, target, z, mask=None):
        ...
        loss = loss_dict["loss"]
        self.last_forward_stats = {
            "diffusion/loss": loss.detach().float().mean(),
        }
        if "mse" in loss_dict:
            self.last_forward_stats["diffusion/mse"] = loss_dict["mse"].detach().float().mean()
        if "vb" in loss_dict:
            self.last_forward_stats["diffusion/vb"] = loss_dict["vb"].detach().float().mean()
        if mask is not None:
    ...
    def sample(self, z, temperature=1.0, cfg=1.0, clip_denoised=False):
        ...
        if not cfg == 1.0:
            ...
            noise = torch.randn(z.shape[0], self.in_channels, device=z.device, dtype=z.dtype)
            model_kwargs = dict(c=z)
