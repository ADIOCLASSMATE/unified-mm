...
CONFIG="${CONFIG:-configs/selfless/imagenet1k_t2i_baseline_80ep_ascend_64npu_bs1024.yaml}"
ACCELERATE_CONFIG="${ACCELERATE_CONFIG:-accelerate_configs/64_npus_4nodes_deepspeed_zero2.yaml}"
RUN_PROJECT="${RUN_PROJECT:-selfless-flow-imagenet1k-t2i-baseline-ascend64-b1024-80ep}"
T2I_VARIANT="${T2I_VARIANT:-baseline}"
...
if [[ ! "${T2I_VARIANT}" =~ ^(baseline|positionwise_head|seq_sigma)$ ]]; then
  echo "ERROR: unsupported T2I_VARIANT=${T2I_VARIANT}" >&2
  exit 10
fi
...
python scripts/validate_ascend_imagenet1k_t2i.py \
  --config "${CONFIG}" \
  --variant "${T2I_VARIANT}" \
...
COMMAND=(
  ...
  "experiment.name=${RUN_PROJECT}-seed42-64x910b-b16ga1-b1024"
