...
Status: **completed; final runtime selected**
...
### 2026-07-25：attempt 0 无效，修复后重试

六个 cell 的首次 Job 都正常训练到约 step `4,490`，随后在第一次周期性 validation
image sampling 同点失败。共同异常为：

```text
ValueError: DF1 cache construction requires context_conditions for every content token.
```

这是共享验证代码的接口错误，不是某个候选架构的训练信号，也不是集群或数值稳定性问题：
DF1 的训练路径和 single-stream generation 路径都为 dynamic content cache 提供 token-wise
condition，但旧的 validation full-sample/probe helper 只传了 content latent、mask 和 position。
因此 attempt 0 的六个 run 全部判为无效，不能进入 selector。

修复保持模型、loss、数据与训练协议不变：validation helper 现在把每个 image token 的
projected Qwen condition 与对应 content latent 一起传给 DF1 cache；flat-query full sample
按 query batch 复制同一条 token-wise condition sequence，probe path 保留原序列。补充
FH0/FH4 回归测试后，从新的 runtime-source manifest 和不可变 snapshot 重提全部六个
seed-42 cell。attempt 0 的 Job、manifest、configs、preflight 与日志保留为 incident evidence。

## 10. 最终结果与架构决定

修复后的六个 seed-42 cell 全部自然完成到 step `35,920`，通过 checkpoint、架构、
fingerprint、10K official evaluation 和 finite-latent postflight。主结果为：

| Cell | FID ↓ | IS ↑ | generation samples/s |
| --- | ---: | ---: | ---: |
| `E2-Q1__DF1-FH0` | 23.6951 | 63.1674 | 0.805 |
| `E2-Q1__DF1-FH4` | **22.9488** | 64.1835 | 0.497 |
| `E2-Q0__DF1-FH0` | 23.3717 | 63.9328 | 0.794 |
| **`E2-Q0__DF1-FH4`** | 23.0140 | **64.9737** | 0.494 |
| `E2b-Q0__DF1-FH0` | 23.6768 | 64.1476 | 0.809 |
| `E2b-Q0__DF1-FH4` | 23.0166 | 64.8187 | 0.492 |

三个 FH4 cell 都进入预注册质量非劣集合。按预注册概念/scaling tie-break，最终选择：

> **`E2-Q0 + DF1-FH4`**

这个组合移除了 observed latent、mask query 和 flow query/content 的 additive absolute
position；Qwen image attention 与 flow attention 都只使用 row/column 2D RoPE。它与最低
FID 相差 `0.0652`，同时取得六个 cell 中最高 IS，满足质量门槛，也是在候选集中位置职责
最统一、最适合扩大 token grid 的架构。

FH4 相对 FH0 在三个 backbone 下都改善 FID（`-0.7463/-0.3577/-0.6602`）和 IS
（`+1.0161/+1.0409/+0.6711`）。当前实现的 generation throughput 约从
`0.794–0.809` 降到 `0.492–0.497 samples/s`；该差异按预注册协议只作为工程诊断，
不否决长期架构。

旧 Q1 anchors 与 fresh Q1 的漂移只用于审计：FH0 为 `+0.1252 FID/-1.6112 IS`，
FH4 为 `-0.0742 FID/-0.4773 IS`。它们来自不同冻结源码，不进入 selector。

完整机器可读结论位于
`output/backbone_flow_head_joint_ablation/evidence/summary_seed42.json`。最终 EMA
checkpoint 位于
`output/selfless-flow-bfh-e2q0-df1-fh4-s42/hf_model-final-ema/model.safetensors`。
本结论决定本版 runtime 默认值，但仍是单 seed system-level screen；论文中的稳定性
表述需要独立多 seed confirmation。
