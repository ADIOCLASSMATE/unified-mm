...
本文件保留实验设计和全部历史候选，仅用于审计。完成 screen 后的 runtime 决议如下。
本次决议是在看到 seed-42 结果后作出的接口收敛选择，因此不追溯声称为预注册结论：

- 架构固定为 `DF1`：content/query 共享 attention 与 MLP，content 逐层更新；
- 活跃 position contract 仅保留 `FH0` 与 `FH4`；
- 默认 baseline 为 `DF1-FH0`，可切换的 pure-RoPE baseline 为 `DF1-FH4`；
- `DF0`、`DF2`、`FH1`、`FH2`、`FH3` 不再是 runtime/config 接口；
- sampling wall time 与 throughput 只作为观测量报告，不再构成候选淘汰门槛。

完整 matched seed-42 结果：

| Cell | FID ↓ | IS ↑ | samples/s | 最终角色 |
| --- | ---: | ---: | ---: | --- |
| DF0-FH0 | 25.0669 | 61.5120 | 1.357 | 历史 static anchor |
| DF0-FH1 | 25.2985 | 60.3427 | 0.918 | 历史 static control |
| DF0-FH4 | 26.1535 | 60.1577 | 0.935 | 历史 static control |
| **DF1-FH0** | **23.5699** | **64.7787** | **0.799** | **默认 baseline** |
| DF1-FH1 | 22.6879 | 64.9852 | 0.473 | 最佳单点指标；hybrid archive-only control |
| **DF1-FH4** | **23.0230** | **64.6608** | **0.489** | **pure-RoPE baseline** |
| DF2-FH0 | 24.9366 | 61.0404 | 0.856 | archive-only |
| DF2-FH1 | 24.5415 | 61.7105 | 0.527 | archive-only |
| DF2-FH4 | 40.8000 | 47.0058 | 0.523 | archive-only failure |

最终选择不声称 `FH1` 数值较差：`DF1-FH1` 是本次单 seed 的 FID/IS 最优点。
保留 `FH0/FH4` 是设计接口收敛决策——二者分别给出 additive-only 与 pure-RoPE
两个原子化端点；hybrid `FH1` 只作为经验上界留在本归档中。所有结论仍限于
ImageNet-100、seed 42、10K evaluation。

### 执行、故障与归档审计

- 六个动态 cell 均使用固定镜像、`8×H100`、priority 4、seed 42 和互不冲突的输出
  目录；主、备用项目并行运行，没有等待主项目结束后才启动备用项目。
- 六个训练 run 均自然完成到 step 35920。恢复后的六个正式 10K evaluator 也都由
  平台自然结束并返回成功，不是人工 stop 后伪装成成功。
- 首轮 evaluator 在生成完成后的汇总阶段确定性触发 `Mapping` 未导入错误。永久修复
  已直接进入活跃 evaluator：显式导入 `collections.abc.Mapping`，并把分布式
  process-group timeout 默认设为 7200 秒且允许环境变量覆盖；对应回归测试覆盖实际
  `init_process_group` 参数。归档 recovery wrapper 和原始故障现场同时保留。
- 平台 job 条目在 checkpoint、metrics、provenance 和哈希核验后已清理；实验文件
  没有删除。数据统一位于 `output/flow_head_ablation/`。
- 归档包含 17 个 run 目录、2022 个数据文件、共 928,041,857,403 bytes，另有 59 个
  归档代码/配置/文档文件和 25 条旧路径迁移记录。完整索引与关键文件 SHA256 在
  `output/flow_head_ablation/relocation_manifest.json`；历史原始 selector 输出保持
  不变，修订后的无效率 gate 汇总位于
  `dynamic_dual_stream_screen/evidence/amended_quality_only_summary.json`。
