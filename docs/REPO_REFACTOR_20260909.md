...
bash script/selfless/validate_repo_refactor_ascend16.sh \
  /absolute/path/to/report all refactor-r1 30
```

必须使用新的 label；该命令依次运行完整回归、16-rank HCCL 文件故障传播、NPU BF16 loss/梯度对照和指定深度的完整保存/恢复验收。深度默认为 30，也可显式传入 `16` 或 `both`。模型验证两次完整的 2,000 张均衡 ImageNet 样本及 400 条 ClimbMix，覆盖冷/热缓存、11 项下游任务、step 2/4/5/6 checkpoint、step 2/4 raw/EMA 配对导出和 final raw/EMA 实际重载生成。EMA 生成还与提交 `1ad1670` 的原生成实现使用相同权重/噪声逐位比较。

## 最终开发机验收 · 2026-09-09

训练和生成实现的验收版本为 `7f90800`，固定开发机 `dev-wjx-ascend`（16×910B，镜像 v1.4）。运行环境未重新安装。用户随后取消 depth16 的完整验收，该运行已停止；它只完成了前两步保存和配对导出，不记为完整通过。以下完整生命周期结果仅指 depth30。
...
ClimbMix 固定验证记录仍标注“可能在训练中见过”，本次没有将其重新解释为独立外部测试集。现有正式作业继续使用此前的 `source-r2`；本轮修改没有部署到这些运行中的作业。

按用户要求，验收结束后删除 smoke 权重、图片、日志和临时目录，包括旧 smoke 输出；此文保留结果和代码版本，不保留 smoke 输出副本。depth16 的取消不算测试失败，也不计入上述完整通过结果。

实际清理 24 个顶层测试目录、4,706 个文件，合计 105.71 GiB（按文件长度计）；排除源码快照和依赖包后，`output/` 下没有残留的 smoke 命名产物。清理后网页再次构建成功，正式模型、结果和图像数量保持一致。开发机已停止。

主要提交便于逐段回退：`05e9ef7` 为实施前基线；`4e0b4c8` 修复单机 caption 锁；`08c1418` 收拢 checkpoint 发布；`2539696` 下沉评测库；`1ad1670` 统一实验身份；`14694d8` 拆分训练职责与恢复错误传播；`463bbe5` 拆分生成 cache/query；`7f90800` 整理研究公共实现及维护入口。跨机器 caption 锁的额外适配已通过 `a970173` 撤回。
